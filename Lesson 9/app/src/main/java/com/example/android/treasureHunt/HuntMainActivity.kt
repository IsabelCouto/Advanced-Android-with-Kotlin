...
class HuntMainActivity : AppCompatActivity() {
    ...
    private val runningQQOrLater = android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.Q
 ...
 */
    ...
    private fun checkDeviceLocationSettingsAndStartGeofence(resolve:Boolean = true) {
        ...
        locationSettingsResponseTask.addOnFailureListener { exception ->
            if (exception is ResolvableApiException && resolve){
                try {
                    exception.startResolutionForResult(this@HuntMainActivity, REQUEST_FOREGROUND_ONLY_PERMISSIONS_REQUEST_CODE)
                }catch (sendEX: IntentSender.SendIntentException){
                    Log.d(TAG, "Error getting location settings resolution" + sendEX.message)
                }
            }else{
                Snackbar.make(binding.activityMapsMain, R.string.location_required_error, Snackbar.LENGTH_INDEFINITE).setAction(android.R.string.ok){
                    checkDeviceLocationSettingsAndStartGeofence()
                }.show()
            }
        }
    ...
    private fun foregroundAndBackgroundLocationPermissionApproved(): Boolean {
        ...
        val backgroundPermissionApproved = if (runningQQOrLater){
